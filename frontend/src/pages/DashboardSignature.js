...
const DashboardSignature = () => {
  ...
  const [previewLoadingPublicId, setPreviewLoadingPublicId] = useState(null);
  ...
  const applyFilters = () => {
    ...
    const notifs = completed
      .slice(-5)
      .map(e => ({ public_id: e.public_id, title: e.title, type: 'normal', time: e.updated_at }));
  ...
  const handlePreview = async (publicId) => {
    setPreviewLoadingPublicId(publicId);
    try {
      ...
      setPreviewLoadingPublicId(null);
    }
  };
            ...
            <>
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {displayedDocuments.map(doc => (
                        <tr key={doc.public_id} className="hover:bg-gray-50 transition-colors duration-150">
                          ...
                          <td className="px-4 lg:px-6 py-4 whitespace-nowrap text-right">
                            <div className="flex items-center justify-end space-x-2">
                              <button
                                onClick={() => handlePreview(doc.public_id)}
                                disabled={previewLoadingPublicId === doc.public_id}
                                className={`p-1 text-gray-400 ${previewLoadingPublicId === doc.public_id ? 'opacity-50 cursor-not-allowed' : 'hover:text-blue-600'} transition-colors duration-200`}
                                title="Prévisualiser"
                              >
                                <Eye className="w-4 h-4" />
                              </button>
                              <button
                                onClick={() => handleDownload(doc.public_id, doc.title)}
              ...
              <div className="block md:hidden">
                <div className="divide-y divide-gray-200">
                  {displayedDocuments.map(doc => (
                    <div key={doc.public_id} className="p-4 hover:bg-gray-50 transition-colors duration-200">
                      {/* En-tête de la carte */}
                      <div className="flex items-start justify-between mb-3">
                        ...
                        {/* Bouton d'expansion */}
                        <button
                          onClick={() => toggleCardExpansion(doc.public_id)}
                          className="p-1 text-gray-400 hover:text-gray-600 transition-colors duration-200"
                          title={expandedCards.has(doc.public_id) ? "Réduire" : "Développer"}
                        >
                          <ChevronDown
                            className={`w-5 h-5 transition-transform duration-200 ${expandedCards.has(doc.public_id) ? 'rotate-180' : ''}`}
                          />
                        </button>
                      </div>
                      ...
                      <div className={`transition-all duration-300 overflow-hidden ${expandedCards.has(doc.public_id) ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
                          ...
                          <div className="flex items-center justify-end space-x-3 pt-2">
                            <button
                              ...
                              className={`flex items-center space-x-1 px-3 py-2 text-xs font-medium rounded-md transition-colors duration-200 ${
                                previewLoadingPublicId === doc.public_id
                      ...
                      <div className={`flex items-center justify-end space-x-2 mt-3 ${expandedCards.has(doc.public_id) ? 'hidden' : 'flex'}`}>
                        <button
                          onClick={() => handlePreview(doc.public_id)}
                          disabled={previewLoadingPublicId === doc.public_id}
                          className={`p-2 text-gray-400 ${previewLoadingPublicId === doc.public_id ? 'opacity-50 cursor-not-allowed' : 'hover:text-blue-600'} transition-colors duration-200`}
                          title="Prévisualiser"
                        >
          ...
          <div className="mt-6 sm:mt-8 bg-white shadow rounded-lg overflow-hidden">
            ...
            <div className="divide-y divide-gray-200">
              {notifications.map(notif => (
                <div key={notif.public_id} className="p-4 sm:p-6 hover:bg-gray-50 transition-colors duration-200">
