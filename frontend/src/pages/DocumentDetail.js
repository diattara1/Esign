...
const DocumentDetail = () => {
  ...
  const handleDownload = async () => {
    try {
      ...
      if (!blobHref) {
        const resolvedPublicId = env?.public_id || publicId;
        if (!resolvedPublicId) {
          throw new Error("Identifiant d'enveloppe manquant");
        }
        const { download_url } = await signatureService.downloadEnvelope(resolvedPublicId);
                ...
                <Document
                  key={
                    env?.status === 'completed'
                      ? `signed-${env?.public_id || 'unknown'}`
                      : `orig-${env?.public_id || 'unknown'}-${selectedDoc?.id || 'single'}`
                  }
                  file={pdfUrl}
