...
export default function SelfSignWizard() {
  ...
  const onFile = (e) => {
    ...
    setFile(f); setPlacement(null); setPdfUrl(f ? URL.createObjectURL(f) : null);
    ...
    <>
      ...
      <div className="p-4 md:p-6 border-b border-gray-200">
        ...
        <div className="relative">
          <input type="file" accept="application/pdf" onChange={onFile} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
          <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:border-emerald-400 hover:bg-emerald-50">
        ...
        {file && (
          <div className="mt-3 flex items-center justify-between p-2 bg-gray-50 rounded text-sm">
            ...
            <button onClick={() => { setFile(null); setPdfUrl(null); setPlacement(null); }} className="ml-2 text-red-500 hover:text-red-700"><FiX className="w-4 h-4" /></button>
          </div>
        )}
      </div>
                    ...
                    <div key={i} className="relative mb-6 bg-white shadow-lg rounded-lg overflow-hidden">
                      ...
                      {placement?.page === i + 1 && (
                        <div className="absolute border-2 border-emerald-500 bg-emerald-100/60 rounded"
                             style={{ top: placement.y * scale, left: `calc(50% - ${pageWidth / 2}px + ${placement.x * scale}px)`, width: placement.width * scale, height: placement.height * scale, zIndex: 20 }}>
